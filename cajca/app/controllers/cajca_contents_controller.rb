class CajcaContentsController < ApplicationController
  ...
  def edit
    @content = CajcaContent.find(params[:id])
  end

  def update
    params[:cajca_content][:existing_pathogens_attributes] ||= {}
    params[:cajca_content][:existing_nodulators_attributes] ||= {}
    params[:cajca_content][:existing_reference_datasets_attributes] ||= {}
    params[:cajca_content][:existing_resources_attributes] ||= {}
    ...
    if @content.update_attributes(params[:cajca_content])
