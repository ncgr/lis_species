...
describe "EditCajcaContents" do
  ...
  before(:each) do
    create_roles
    @content = FactoryGirl.create(:cajca_content)
  ...
  it "as superuser" do
    ...
    current_path.should eq(edit_cajca_content_path(@content.id))
    ...
    fill_in "cajca_content_new_nodulators_attributes__species", :with => "species"
    fill_in "cajca_content_new_nodulators_attributes__ncbi_taxon_id", :with => "1234"
    ...
    fill_in "cajca_content_new_pathogens_attributes__name", :with => "pathogen"
    fill_in "cajca_content_new_pathogens_attributes__ncbi_taxon_id", :with => "777"
    ...
    fill_in "cajca_content_new_resources_attributes__resource_type", :with => "resource"
    fill_in "cajca_content_new_resources_attributes__description", :with => "resource description"
    fill_in "cajca_content_new_resources_attributes__url", :with => "http://google.com"
    select "Yes", :from => "cajca_content_new_resources_attributes__is_public"

    click_button "Add Reference Dataset"
    select "Genomic Sequence", :from => "cajca_content_new_reference_datasets_attributes__reference_dataset_type"
    fill_in "cajca_content_new_reference_datasets_attributes__description", :with => "brief"
    fill_in "cajca_content_new_reference_datasets_attributes__source", :with => "Mr. T"
    fill_in "cajca_content_new_reference_datasets_attributes__url", :with => "http://google.com"
    select "Yes", :from => "cajca_content_new_reference_datasets_attributes__is_public"
  ...
  it "should not be able as editor" do
    ...
    visit edit_cajca_content_path(@content.id)
