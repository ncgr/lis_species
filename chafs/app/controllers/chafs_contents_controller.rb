...
class ChafsContentsController < ApplicationController
  ...
  def edit
    @content = ChafsContent.find(params[:id])
  end
  
  def update
    params[:chafs_content][:existing_pathogens_attributes] ||= {}
    params[:chafs_content][:existing_nodulators_attributes] ||= {}
    params[:chafs_content][:existing_reference_datasets_attributes] ||= {}
    params[:chafs_content][:existing_resources_attributes] ||= {}
    ...
    if @content.update_attributes(params[:chafs_content])
