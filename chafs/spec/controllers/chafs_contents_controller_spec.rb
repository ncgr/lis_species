...
describe ChafsContentsController do
  
  def mock_chafs_content(stubs={})
    @mock_chafs_content ||= mock_model(ChafsContent, stubs).as_null_object
  end
  ...
  describe "GET index without logging in" do
    before(:each) do
      @chafs = Factory.build(:chafs_content)
    end
  ...
  describe "GET edit and PUT update chafs_content while logged in as superuser" do
    login_superuser
    before(:each) do
      @chafs = Factory.build(:chafs_content)
      ChafsContent.should_receive(:find).with("21").and_return(@chafs)
    end
    it "should find chafs_content and return object" do
      get :edit, :id => "21"
      response.should render_template('edit')
    end
    it "should update object" do
      put :update, :id => "21", :chafs_content => {}
      response.should be_redirect
    end
  end
  
  describe "GET edit and PUT update chafs_content while logged in as admin" do
    login_admin
    before(:each) do
      @chafs = Factory.build(:chafs_content)
      ChafsContent.should_receive(:find).with("212").and_return(@chafs)
    end
    it "should find chafs_content and return object" do
      get :edit, :id => "212"
      response.should render_template('edit')
    ...
    it "should update object" do
      put :update, :id => "212", :chafs_content => {}
      response.should be_redirect
    end
  end
