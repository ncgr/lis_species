...
describe GlymaGenomeSummary do
  ...
  it "has one record" do
    Factory.create(:glyma_genome_summary)
    GlymaGenomeSummary.count.should == 1
  end
  
  it "removes % after self.gc_content_genome" do
    @glyma_genome_summary = GlymaGenomeSummary.create!({:glyma_content_id => 1, :gc_content_genome => '22.5%'})
    @glyma_genome_summary.gc_content_genome.should == '22.5'
  end
  
  it "removes % after self.gc_content_transcriptome" do
    @glyma_genome_summary = GlymaGenomeSummary.create!({:glyma_content_id => 1, :gc_content_transcriptome => '78%'})
