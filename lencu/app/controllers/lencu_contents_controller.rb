class LencuContentsController < ApplicationController
  ...
  def edit
    @content = LencuContent.find(params[:id])
  end

  def update
    params[:lencu_content][:existing_pathogens_attributes] ||= {}
    params[:lencu_content][:existing_nodulators_attributes] ||= {}
    params[:lencu_content][:existing_reference_datasets_attributes] ||= {}
    params[:lencu_content][:existing_resources_attributes] ||= {}
    ...
    if @content.update_attributes(params[:lencu_content])
