...
describe LupalContentsController do
  
  def mock_lupal_content(stubs={})
    @mock_lupal_content ||= mock_model(LupalContent, stubs).as_null_object
  end
  ...
  describe "GET index without logging in" do
    before(:each) do
      @lupal = Factory.build(:lupal_content)
    end
  ...
  describe "GET edit and PUT update lupal_content while logged in as superuser" do
    login_superuser
    before(:each) do
      @lupal = Factory.build(:lupal_content)
      LupalContent.should_receive(:find).with("21").and_return(@lupal)
    end
    it "should find lupal_content and return object" do
      get :edit, :id => "21"
      response.should render_template('edit')
    end
    it "should update object" do
      put :update, :id => "21", :lupal_content => {}
      response.should be_redirect
    end
  end
  
  describe "GET edit and PUT update lupal_content while logged in as admin" do
    login_admin
    before(:each) do
      @lupal = Factory.build(:lupal_content)
      LupalContent.should_receive(:find).with("212").and_return(@lupal)
    end
    it "should find lupal_content and return object" do
      get :edit, :id => "212"
      response.should render_template('edit')
    ...
    it "should update object" do
      put :update, :id => "212", :lupal_content => {}
      response.should be_redirect
    end
  end
