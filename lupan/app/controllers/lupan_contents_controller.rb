...
class LupanContentsController < ApplicationController
  ...
  def edit
    @content = LupanContent.find(params[:id])
  end
  
  def update
    params[:lupan_content][:existing_pathogens_attributes] ||= {}
    params[:lupan_content][:existing_nodulators_attributes] ||= {}
    params[:lupan_content][:existing_reference_datasets_attributes] ||= {}
    params[:lupan_content][:existing_resources_attributes] ||= {}
    ...
    if @content.update_attributes(params[:lupan_content])
