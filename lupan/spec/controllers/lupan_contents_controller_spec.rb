...
describe LupanContentsController do
  
  def mock_lupan_content(stubs={})
    @mock_lupan_content ||= mock_model(LupanContent, stubs).as_null_object
  end
  ...
  describe "GET index without logging in" do
    before(:each) do
      @lupan = Factory.build(:lupan_content)
    end
  ...
  describe "GET edit and PUT update lupan_content while logged in as superuser" do
    login_superuser
    before(:each) do
      @lupan = Factory.build(:lupan_content)
      LupanContent.should_receive(:find).with("21").and_return(@lupan)
    end
    it "should find lupan_content and return object" do
      get :edit, :id => "21"
      response.should render_template('edit')
    end
    it "should update object" do
      put :update, :id => "21", :lupan_content => {}
      response.should be_redirect
    end
  end
  
  describe "GET edit and PUT update lupan_content while logged in as admin" do
    login_admin
    before(:each) do
      @lupan = Factory.build(:lupan_content)
      LupanContent.should_receive(:find).with("212").and_return(@lupan)
    end
    it "should find lupan_content and return object" do
      get :edit, :id => "212"
      response.should render_template('edit')
    ...
    it "should update object" do
      put :update, :id => "212", :lupan_content => {}
      response.should be_redirect
    end
  end
