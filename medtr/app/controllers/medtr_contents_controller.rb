...
class MedtrContentsController < ApplicationController
  ...
  def edit
    @content = MedtrContent.find(params[:id])
  end

  def update
    params[:medtr_content][:existing_pathogens_attributes] ||= {}
    params[:medtr_content][:existing_nodulators_attributes] ||= {}
    params[:medtr_content][:existing_reference_datasets_attributes] ||= {}
    params[:medtr_content][:existing_resources_attributes] ||= {}
    ...
    if @content.update_attributes(params[:medtr_content])
