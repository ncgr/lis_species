...
describe PhacnContentsController do
  
  def mock_phacn_content(stubs={})
    @mock_phacn_content ||= mock_model(PhacnContent, stubs).as_null_object
  end
  ...
  describe "GET index without logging in" do
    before(:each) do
      @phacn = Factory.build(:phacn_content)
    end
  ...
  describe "GET edit and PUT update phacn_content while logged in as superuser" do
    login_superuser
    before(:each) do
      @phacn = Factory.build(:phacn_content)
      PhacnContent.should_receive(:find).with("21").and_return(@phacn)
    end
    it "should find phacn_content and return object" do
      get :edit, :id => "21"
      response.should render_template('edit')
    end
    it "should update object" do
      put :update, :id => "21", :phacn_content => {}
      response.should be_redirect
    end
  end
  
  describe "GET edit and PUT update phacn_content while logged in as admin" do
    login_admin
    before(:each) do
      @phacn = Factory.build(:phacn_content)
      PhacnContent.should_receive(:find).with("212").and_return(@phacn)
    end
    it "should find phacn_content and return object" do
      get :edit, :id => "212"
      response.should render_template('edit')
    ...
    it "should update object" do
      put :update, :id => "212", :phacn_content => {}
      response.should be_redirect
    end
  end
