...
class TriprContentsController < ApplicationController
  ...
  def edit
    @content = TriprContent.find(params[:id])
  end

  def update
    params[:tripr_content][:existing_pathogens_attributes] ||= {}
    params[:tripr_content][:existing_nodulators_attributes] ||= {}
    params[:tripr_content][:existing_reference_datasets_attributes] ||= {}
    params[:tripr_content][:existing_resources_attributes] ||= {}
    ...
    if @content.update_attributes(params[:tripr_content])
