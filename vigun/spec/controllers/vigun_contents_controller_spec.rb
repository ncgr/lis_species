...
describe VigunContentsController do
  
  def mock_vigun_content(stubs={})
    @mock_vigun_content ||= mock_model(VigunContent, stubs).as_null_object
  end
  ...
  describe "GET index without logging in" do
    before(:each) do
      @vigun = Factory.build(:vigun_content)
    end
  ...
  describe "GET edit and PUT update vigun_content while logged in as superuser" do
    login_superuser
    before(:each) do
      @vigun = Factory.build(:vigun_content)
      VigunContent.should_receive(:find).with("21").and_return(@vigun)
    end
    it "should find vigun_content and return object" do
      get :edit, :id => "21"
      response.should render_template('edit')
    end
    it "should update object" do
      put :update, :id => "21", :vigun_content => {}
      response.should be_redirect
    end
  end
  
  describe "GET edit and PUT update vigun_content while logged in as admin" do
    login_admin
    before(:each) do
      @vigun = Factory.build(:vigun_content)
      VigunContent.should_receive(:find).with("212").and_return(@vigun)
    end
    it "should find vigun_content and return object" do
      get :edit, :id => "212"
      response.should render_template('edit')
    ...
    it "should update object" do
      put :update, :id => "212", :vigun_content => {}
      response.should be_redirect
    end
  end
